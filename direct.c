...
KILLERHASHVALUE* getKillerHashValue();
...
void solve_direct(DIR_SOL* dsol, BOARD* startpos)
{
    ...
    if ((g_actual == true) && (g_moves == 1)) {
    ...
    } else if ((g_actual == true) || (g_tries == true)) {
        for (m = 1; m < g_moves; m++) {
            int ct;
            BOARD* b;
            BOARDLIST* ml;
        ...
        if (shortsol == false) {
            ...
            if ((g_tries == true) && (g_trivtries == false)) {
    ...
    if ((shortsol == false) && (g_moves > 1) && (g_set == true)
...
static BOARDLIST* norm_blackMidMove(BOARD* inBrd, int move)
{
    ...
    if (refutationFound == false) {
        ...
        if ((state != SETPLAY) && (ct != 0)
                && (g_shortvars == false) && (minStip != maxStip)) {
...
static BOARDLIST* blackMove(BOARD* inBrd)
{
    ...
    if (refuts <= g_refuts) {
        ...
        if ((g_shortvars == false) && (minStip != maxStip)) {
            weedOutShortVars(bml, (unsigned char) g_moves);
        }
    } else {
        bml->minStip = NOSTIP;
        bml->maxStip = NOSTIP;
...
static BOARDLIST* norm_final_move(BOARD* inBrd, int move)
{
    ...
    int ct;
    assert(inBrd != NULL);
    wml = generateWhiteBoardlist(inBrd, g_moves);

    DL_FOREACH_SAFE(wml->vektor, bd, tmp) {
        if (bd->check == true) {
            bml = generateRefutations(bd, g_moves);
            assert(bml != NULL);
            DL_COUNT(bml->vektor, tmp1, ct);
    ...
    if (stipAchieved == true) {
        BOARDLIST* uml = generateWhiteBoardlist(inBrd, g_moves);
        BOARD* b;
        DL_FOREACH(wml->vektor, b) {
            qualifyMove(uml, b);
        }
    ...
    wml->maxStip =
        (stipAchieved == true) ? (unsigned char) g_moves : NOSTIP;
    wml->minStip =
        (stipAchieved == true) ? (unsigned char) g_moves : NOSTIP;
    wml->stipIn =
        (stipAchieved == true) ? (unsigned char) g_moves : NOSTIP;
        ...
        if (bbl != NULL) {
            ...
            if ((move + 1) == g_moves) {
                if ((check == false) && (bbl->legalMoves > 1)) {
                    tbl = norm_final_move(wb, g_moves);
                    assert(tbl != NULL);
                    DL_COUNT(tbl->vektor, tmp, ct);

                    if (ct > 0) {
                        wb->threat = tbl;
                        weedNonDefences(tbl, bbl, g_fleck);
                    } else {
                        freeBoardlist(tbl);
                    }
                }
    ...
    DL_FOREACH_SAFE(wml->vektor, m, tmp) {
        if ((state != THREATS) && (shortStipAchieved == true)) {
            ...
            if ((ct == 0) && (m->check == false)) {
                ...
                if (minStip < (unsigned char) g_moves) {
                    shortStipAchieved = true;
                }
            }
        }
